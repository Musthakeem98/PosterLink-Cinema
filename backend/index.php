...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Validate form data
    if (empty($input["first_name"]) || empty($input["last_name"]) || empty($input["message"]) || empty($input["email"])) {
        echo json_encode(array("success" => false,"status" => "feilds missed", "message" => "Please fill in all required fields."));
        exit;
    }

    if (($input["agree_terms"] == false))  {
        echo json_encode(array("success" => false,"status" => "terms missed", "message" => "Please agree to the Terms & Conditions"));
        exit;
    }
